...
| **用户/角色/权限** | RBAC 权限体系、角色管理、权限分配 | ✅ |
| **MQ Consumer** | 出单→佣金计算、理赔→赔付通知 | ✅ |

### 规划中模块（详见 [开发计划](docs/Covex补充计划20260706.md)）

| 模块 | 说明 | 计划期次 | 状态 |
|---|---|---|:---:|
| **产品模板工厂** | 产品库三层架构 + 6 个预置模板 + 3 步向导创建 | P0.5 | 🔨 待开发 |
| **13 角色权限重构** | 完整保险公司组织架构（4 组 13 角色 42 路由） | P1.5 | 🔨 待开发 |
| **审批流程** | 渠道审批、产品发布审批、理赔多级审核、佣金结算审批 | P2 | 🔨 待开发 |
| **规则引擎服务** | 独立微服务（8081），6 域 42 条规则，版本追溯，热更新 | P2.5 | 🔨 待开发 |
| **架构优化** | 费率表强制校验、MQ 延迟消息、权限体系落地 | P3 | 🔨 待开发 |
...
| 开发计划 | `docs/Covex补充计划20260706.md` | 7 期 36 天开发计划 |
...
- **运营域**（29 张表）：客户、投保单、保单、理赔、佣金、渠道、权限等

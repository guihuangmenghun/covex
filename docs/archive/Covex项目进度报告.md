...
- @RequiresPermission 方法级权限校验（15 个关键接口）
- PermissionCacheService 权限 Redis 缓存（30 分钟 TTL）
- DataPermissionInterceptor 数据权限隔离
- Redisson 分布式锁（支付回调/赔付/角色分配防并发）

---

## 七、全量审计补全（2026-07-05）

> 由代码审计发现开发计划标记 [x] 但实际未落地的遗漏项，共 10 个任务已全部实现并通过编译验证。

| # | 任务 | 类型 | 涉及文件 |
|---|---|---|---|
| 1 | PolicyIssuedConsumer — 出单消息自动计算佣金 | MQ Consumer | service/mq/PolicyIssuedConsumer.java（新建） |
| 2 | 支付超时改用 RocketMQ 延迟消息（30min） | MQ Producer/Consumer | PaymentService.java + service/mq/PaymentTimeoutConsumer.java（新建） |
| 3 | 支付回调丢失补偿定时任务（每天凌晨2点） | @Scheduled | PaymentCompensationService.java（新建） |
| 4 | PermissionCacheService — 权限 Redis 缓存 | 缓存 | PermissionCacheService.java（新建）+ UserService/RoleService/DataScopeService 改造 |
| 5 | @RequiresPermission 注解 + AOP 权限校验 | 注解+AOP | RequiresPermission.java（新建）+ PermissionAspect.java（新建）+ 8个 Controller 加注解 |
| 6 | DataPermissionInterceptor 数据权限拦截器 | MyBatis 拦截器 | DataPermissionInterceptor.java（新建）+ MybatisPlusConfig 注册 |
| 7 | @Transactional 补全 rollbackFor = Exception.class | 全局替换 | 10+ 个 Service 文件，共 30+ 处替换 |
| 8 | 序号生成改用 Redis INCR（替代 nanoTime） | 序号生成 | ProposalService/PaymentService/ClaimService/ClaimPaymentService |
| 9 | Redisson 分布式锁（支付回调/赔付/角色分配） | 分布式锁 | PaymentService/ClaimPaymentService/UserService |
| 10 | CLAIM_PAID Consumer — 赔付通知日志 | MQ Consumer | service/mq/ClaimPaidConsumer.java（新建） |
